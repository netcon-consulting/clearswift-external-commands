...
from netcon import ParserArgs, get_config, write_log, get_url_list, domain_blacklisted, CHARSET_UTF8
...
def main(args):
    ...
    for qr_code in list_qr:
        ...
        if text is not None and re.search(PATTERN_URL, text) is not None:
            try:
                set_blacklist = { re.compile(r"^{}$".format(url.replace(".", r"\.").replace("*", ".*"))) for url in get_url_list(config.name_url_blacklist) }
                set_whitelist = { re.compile(r"^{}$".format(url.replace(".", r"\.").replace("*", ".*"))) for url in get_url_list(config.name_url_whitelist) }
            except Exception as ex:
                write_log(args.log, ex)
            ...
            for match in re.finditer(PATTERN_URL, text):
                ...
                for pattern in set_whitelist:
                    ...
                    for pattern in set_blacklist:
                        if re.search(pattern, url) is not None:
                            write_log(args.log, "'{}' listed on '{}'".format(url, args.name_url_blacklist))
